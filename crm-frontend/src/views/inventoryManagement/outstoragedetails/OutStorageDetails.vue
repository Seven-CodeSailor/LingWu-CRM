<template>
  <el-card>
    ...
    <header>
      <h3>
        <slot name="ico"></slot>
        <div style="margin-left: 8px">出库明细</div>
      </h3>
    </header>
    <!-- 操作搜索栏 -->
    <section class="menu">
    ...
    <el-table
      ...
      table-layout="auto"
      :data="outStorageDetails.tableData"
    ...
    <el-pagination
      ...
      :total="outStorageDetails.totalTable"
...
import useOutStorageDetails from '@/stores/inventory/stockstoragedetails.js'
import {
  queryOutStock,
...
const outStorageDetails = useOutStorageDetails()

// 初始化数据
const initRecords = (currentPage, pageSize, remarks) => {
  queryOutStock(
    currentPage,
    pageSize,
    remarks,
    (response) => {
      outStorageDetails.setTableData(response.data.rows)
      outStorageDetails.totalTable = response.data.total
    },
    (error) => {
